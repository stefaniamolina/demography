...
CL <- read.csv("Taiwan_ASFR.csv")
head(CL)

CL <- CL %>%
  mutate(Code = "CL") %>%  # include code
  mutate(ID = paste(Code, Year, Age, Cohort, sep = "")) %>% # include ID
  select(ID, Code, everything()) # rearrange columns

CL[CL == "."] <- NA 
CL <- na.omit(CL) #remove NAs

CL$Cohort <- as.integer(CL$Cohort)
...
POS <- rbind.fill(apply(CL,1,LexTriCoords))
ASFR <- data.frame(ID = as.factor(CL$ID),ASFR = as.numeric(CL$ASFR)) 
datapoly <- merge(ASFR, POS, by=c("ID"))
datapoly <- datapoly[datapoly$ASFR > 0,]

a <- -rev(unique(CL$Cohort)[unique(CL$Cohort)%%5==0])
...
CLplot <- (LexFx <-  ggplot(datapoly, aes(x=Year, y=Age)) + geom_polygon(aes(fill=ASFR, group=ID)) +
             scale_fill_gradientn(colors= cols,limits=c(0,.25),breaks=brks,labels=labs,space="Lab") + 
             theme_bw() +
             coord_equal(ratio = 1) + 
             scale_x_continuous(expand=c(0,0),breaks=seq(1992,2005,by=10),labels=seq(1992,2005,by=10),limits=c(1992,2005)) + 
             scale_y_continuous(expand=c(0,0),breaks=seq(15,55,by=10),labels=seq(15,55,by=10),limits=c(12,56)) +
             geom_vline(xintercept=seq(1992,2005,by=10),colour="#80808030") +
             geom_hline(yintercept=seq(15,55,by=10),colour="#80808030") +
             geom_abline(aes(intercept=a, slope=b),data=CL,colour="#80808030") +
             ggtitle("Chile")
)
