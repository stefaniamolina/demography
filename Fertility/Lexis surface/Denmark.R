...
DK <- read.csv("Denmark_ASFR.csv")
head(DK)

DK <- DK %>%
  mutate(Code = "DK") %>%  # include code
  mutate(ID = paste(Code, Year, Age, Cohort, sep = "")) %>% # include ID
  select(ID, Code, everything()) # rearrange columns

DK[DK == "."] <- NA 
DK <- na.omit(DK) #remove NAs

DK$Cohort <- as.integer(DK$Cohort)
...
POS <- rbind.fill(apply(DK,1,LexTriCoords))
ASFR <- data.frame(ID = as.factor(DK$ID),ASFR = as.numeric(DK$ASFR)) 
datapoly <- merge(ASFR, POS, by=c("ID"))
datapoly <- datapoly[datapoly$ASFR > 0,]

a <- -rev(unique(DK$Cohort)[unique(DK$Cohort)%%5==0])
...
DKplot <- (LexFx <-  ggplot(datapoly, aes(x=Year, y=Age)) + geom_polygon(aes(fill=ASFR, group=ID)) +
             scale_fill_gradientn(colors= cols,limits=c(0,.25),breaks=brks,labels=labs,space="Lab") + 
             theme_bw() +
             coord_equal(ratio = 1) + 
             scale_x_continuous(expand=c(0,0),breaks=seq(1920,2020,by=10),labels=seq(1920,2020,by=10),limits=c(1920,2020)) + 
             scale_y_continuous(expand=c(0,0),breaks=seq(15,55,by=10),labels=seq(15,55,by=10),limits=c(12,56)) +
             geom_vline(xintercept=seq(1920,2020,by=10),colour="#80808030") +
             geom_hline(yintercept=seq(15,55,by=10),colour="#80808030") +
             geom_abline(aes(intercept=a, slope=b),data=DK,colour="#80808030") +
             ggtitle("Denmark")
)
