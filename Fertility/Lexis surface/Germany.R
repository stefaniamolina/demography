...
DE <- read.csv("Germany_ASFR.csv")
head(DE)

DE <- DE %>%
  mutate(Code = "DE") %>%  # include code
  mutate(ID = paste(Code, Year, Age, Cohort, sep = "")) %>% # include ID
  select(ID, Code, everything()) # rearrange columns

DE[DE == "."] <- NA 
DE <- na.omit(DE) #remove NAs

DE$Cohort <- as.integer(DE$Cohort)
...
POS <- rbind.fill(apply(DE,1,LexTriCoords))
ASFR <- data.frame(ID = as.factor(DE$ID),ASFR = as.numeric(DE$ASFR)) 
datapoly <- merge(ASFR, POS, by=c("ID"))
datapoly <- datapoly[datapoly$ASFR > 0,]

a <- -rev(unique(DE$Cohort)[unique(DE$Cohort)%%5==0])
...
DE_plot <- (LexFx <-  ggplot(datapoly, aes(x=Year, y=Age)) + geom_polygon(aes(fill=ASFR, group=ID)) +
    scale_fill_gradientn(colors= cols,limits=c(0,.25),breaks=brks,labels=labs,space="Lab") + 
    theme_bw() +
    coord_equal(ratio = 1) + 
    scale_x_continuous(expand=c(0,0),breaks=seq(1956,2017,by=10),labels=seq(1956,2017,by=10),limits=c(1956,2017)) + 
    scale_y_continuous(expand=c(0,0),breaks=seq(15,55,by=10),labels=seq(15,55,by=10),limits=c(12,56)) +
    geom_vline(xintercept=seq(1956,2017,by=10),colour="#80808030") +
    geom_hline(yintercept=seq(15,55,by=10),colour="#80808030") +
    geom_abline(aes(intercept=a, slope=b),data=DE,colour="#80808030") +
    ggtitle("Germany")
)
