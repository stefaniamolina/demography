...
IT <- read.csv("Italy_ASFR.csv")
head(IT)

IT <- IT %>%
  mutate(Code = "IT") %>%  # include code
  mutate(ID = paste(Code, Year, Age, Cohort, sep = "")) %>% # include ID
  select(ID, Code, everything()) # rearrange columns

IT[IT == "."] <- NA 
IT <- na.omit(IT) #remove NAs

IT$Cohort <- as.integer(IT$Cohort)
...
POS <- rbind.fill(apply(IT,1,LexTriCoords))
ASFR <- data.frame(ID = as.factor(IT$ID),ASFR = as.numeric(IT$ASFR)) 
datapoly <- merge(ASFR, POS, by=c("ID"))
datapoly <- datapoly[datapoly$ASFR > 0,]

a <- -rev(unique(IT$Cohort)[unique(IT$Cohort)%%5==0])
...
ITplot <- (LexFx <-  ggplot(datapoly, aes(x=Year, y=Age)) + geom_polygon(aes(fill=ASFR, group=ID)) +
             ...
             geom_abline(aes(intercept=a, slope=b),data=IT,colour="#80808030") +
             ggtitle("Italy")
)
