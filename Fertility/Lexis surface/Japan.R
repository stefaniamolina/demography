...
JP <- read.csv("Japan_ASFR.csv")
head(JP)

JP <- JP %>%
  mutate(Code = "JP") %>%  # include code
  mutate(ID = paste(Code, Year, Age, Cohort, sep = "")) %>% # include ID
  select(ID, Code, everything()) # rearrange columns

JP[JP == "."] <- NA 
JP <- na.omit(JP) #remove NAs

JP$Cohort <- as.integer(JP$Cohort)
...
POS <- rbind.fill(apply(JP,1,LexTriCoords))
ASFR <- data.frame(ID = as.factor(JP$ID),ASFR = as.numeric(JP$ASFR)) 
datapoly <- merge(ASFR, POS, by=c("ID"))
datapoly <- datapoly[datapoly$ASFR > 0,]

a <- -rev(unique(JP$Cohort)[unique(JP$Cohort)%%5==0])
...
JPplot <- (LexFx <-  ggplot(datapoly, aes(x=Year, y=Age)) + geom_polygon(aes(fill=ASFR, group=ID)) +
             scale_fill_gradientn(colors= cols,limits=c(0,.25),breaks=brks,labels=labs,space="Lab") + 
             theme_bw() +
             coord_equal(ratio = 1) + 
             scale_x_continuous(expand=c(0,0),breaks=seq(1952,2018,by=10),labels=seq(1952,2018,by=10),limits=c(1952,2018)) + 
             scale_y_continuous(expand=c(0,0),breaks=seq(15,55,by=10),labels=seq(15,55,by=10),limits=c(12,56)) +
             geom_vline(xintercept=seq(1952,2018,by=10),colour="#80808030") +
             geom_hline(yintercept=seq(15,55,by=10),colour="#80808030") +
             geom_abline(aes(intercept=a, slope=b),data=JP,colour="#80808030") +
             ggtitle("Japan")
)
