...
PL <- read.csv("Poland_ASFR.csv")
head(PL)

PL <- PL %>%
  mutate(Code = "PL") %>%  # include code
  mutate(ID = paste(Code, Year, Age, Cohort, sep = "")) %>% # include ID
  select(ID, Code, everything()) # rearrange columns

PL[PL == "."] <- NA 
PL <- na.omit(PL) #remove NAs

PL$Cohort <- as.integer(PL$Cohort)
...
POS <- rbind.fill(apply(PL,1,LexTriCoords))
ASFR <- data.frame(ID = as.factor(PL$ID),ASFR = as.numeric(PL$ASFR)) 
datapoly <- merge(ASFR, POS, by=c("ID"))
datapoly <- datapoly[datapoly$ASFR > 0,]

a <- -rev(unique(PL$Cohort)[unique(PL$Cohort)%%5==0])
...
PLplot <- (LexFx <-  ggplot(datapoly, aes(x=Year, y=Age)) + geom_polygon(aes(fill=ASFR, group=ID)) +
             scale_fill_gradientn(colors= cols,limits=c(0,.25),breaks=brks,labels=labs,space="Lab") + 
             theme_bw() +
             coord_equal(ratio = 1) + 
             scale_x_continuous(expand=c(0,0),breaks=seq(1971,2016,by=10),labels=seq(1971,2016,by=10),limits=c(1971,2016)) + 
             scale_y_continuous(expand=c(0,0),breaks=seq(15,55,by=10),labels=seq(15,55,by=10),limits=c(12,56)) +
             geom_vline(xintercept=seq(1971,2016,by=10),colour="#80808030") +
             geom_hline(yintercept=seq(15,55,by=10),colour="#80808030") +
             geom_abline(aes(intercept=a, slope=b),data=PL,colour="#80808030") +
             ggtitle("Poland")
)
