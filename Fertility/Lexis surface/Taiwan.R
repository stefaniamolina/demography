...
TW <- read.csv("Taiwan_ASFR.csv")
head(TW)

TW <- TW %>%
  mutate(Code = "TW") %>%  # include code
  mutate(ID = paste(Code, Year, Age, Cohort, sep = "")) %>% # include ID
  select(ID, Code, everything()) # rearrange columns

TW[TW == "."] <- NA 
TW <- na.omit(TW) #remove NAs

TW$Cohort <- as.integer(TW$Cohort)
...
POS <- rbind.fill(apply(TW,1,LexTriCoords))
ASFR <- data.frame(ID = as.factor(TW$ID),ASFR = as.numeric(TW$ASFR)) 
datapoly <- merge(ASFR, POS, by=c("ID"))
datapoly <- datapoly[datapoly$ASFR > 0,]

a <- -rev(unique(TW$Cohort)[unique(TW$Cohort)%%5==0])
...
TWplot <- (LexFx <-  ggplot(datapoly, aes(x=Year, y=Age)) + geom_polygon(aes(fill=ASFR, group=ID)) +
             scale_fill_gradientn(colors= cols,limits=c(0,.25),breaks=brks,labels=labs,space="Lab") + 
             theme_bw() +
             coord_equal(ratio = 1) + 
             scale_x_continuous(expand=c(0,0),breaks=seq(1980,2019,by=10),labels=seq(1980,2019,by=10),limits=c(1980,2019)) + 
             scale_y_continuous(expand=c(0,0),breaks=seq(15,55,by=10),labels=seq(15,55,by=10),limits=c(12,56)) +
             geom_vline(xintercept=seq(1980,2019,by=10),colour="#80808030") +
             geom_hline(yintercept=seq(15,55,by=10),colour="#80808030") +
             geom_abline(aes(intercept=a, slope=b),data=TW,colour="#80808030") +
             ggtitle("Taiwan")
)
