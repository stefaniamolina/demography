...
US <- read.csv("US_ASFR.csv")
head(US)

US <- US %>%
  mutate(Code = "US") %>%  # include code
...
US$Cohort <- as.integer(US$Cohort)
...
POS <- rbind.fill(apply(US,1,LexTriCoords))
ASFR <- data.frame(ID = as.factor(US$ID),ASFR = as.numeric(US$ASFR)) 
datapoly <- merge(ASFR, POS, by=c("ID"))
datapoly <- datapoly[datapoly$ASFR > 0,]

a <- -rev(unique(US$Cohort)[unique(US$Cohort)%%5==0])
...
USplot <- (LexFx <-  ggplot(datapoly, aes(x=Year, y=Age)) + geom_polygon(aes(fill=ASFR, group=ID)) +
             scale_fill_gradientn(colors= cols,limits=c(0,.25),breaks=brks,labels=labs,space="Lab") + 
             theme_bw() +
             coord_equal(ratio = 1) + 
             scale_x_continuous(expand=c(0,0),breaks=seq(1963,2019,by=10),labels=seq(1963,2019,by=10),limits=c(1963,2019)) + 
             scale_y_continuous(expand=c(0,0),breaks=seq(15,55,by=10),labels=seq(15,55,by=10),limits=c(12,56)) +
             geom_vline(xintercept=seq(1963,2019,by=10),colour="#80808030") +
             geom_hline(yintercept=seq(15,55,by=10),colour="#80808030") +
             geom_abline(aes(intercept=a, slope=b),data=US,colour="#80808030") +
