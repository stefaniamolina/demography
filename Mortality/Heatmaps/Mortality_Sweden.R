...
sweden <- read_table(paste0("SWE.Mx_1x1.txt"), skip = 2, na = ".") %>% clean_names()
sweden$age <- as.integer(recode(sweden$age, "110+" = "110"))

sweden <- sweden %>% mutate(ratio = male / female,
                              deciles = cut(ratio, breaks = quantile(ratio, probs = seq(0, 1, 0.1), na.rm = TRUE)),
                              pct_diff = ((male - female) / (male + female))*100)


pfemale <- ggplot(subset(sweden, age < 101), aes(x = year, y = age, fill = ntile(female, 100)))
pfemale_out <- pfemale + geom_raster() +
  scale_fill_viridis_c(option = "F", direction = -1) +
  scale_x_continuous(breaks = seq(1760, 2015, by = 15)) +
...
### sweden male
###--------------------------------------------------
pmale <- ggplot(data = subset(sweden, age < 101), 
                mapping = aes(x = year, 
...
sweden_mortality <- ggarrange(pfemale_out, pmale_out,
                               ncol = 1, nrow = 2,
                               common.legend = TRUE,
                               legend = "bottom")
